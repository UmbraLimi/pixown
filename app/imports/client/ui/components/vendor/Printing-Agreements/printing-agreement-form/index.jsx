...
@inject('store')
@observer
class Printing_Agreement_Form extends React.Component {
	constructor(props) {
		super(props)
	...
	render() {
		const { Manager:paManager, events, formStore } = this.props
		const pass_thru = {
			manager: paManager, 
			formStore: formStore,
		...
		return (
			<div>
				...
				<Field 	{...pass_thru} 					
						...
						label="Studio"
						tabIndex={this.tabIndexGen.next().value}
						title={Vendor_Masthead_Spec(paManager)}
				/>
				<Field 	{...pass_thru} 					
						id='title'
						label="Title"
...
export { Printing_Agreement_Form }
