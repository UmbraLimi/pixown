...
@inject('store')
@observer
class Retouching_Agreements_Listing extends React.Component {
	render() {
		const { Manager:raListManager, events } = this.props
    const { raManagerList } = raListManager

    if (raManagerList.length === 0) { 
      return (<NoAgreements_> no agreements available </NoAgreements_>) 
    } // -> -> -> -> //

 		return (
      <Listing_   className='Retouching_Agreements_Listing_' >
      {
        __.map(raManagerList, (raManager, num) => {
          const agreement = raManager.record
          return (
            <Row_ 
                className='Row_'
            ...
            >
              ...
              <Details_ className='Agreement_Details_' >
                ...
                <Detail_Row   
                    label={ "Service Codes" }
                    valueList={ [agreement.services_list.service_code] }
                />
                <Detail_Row   
                    label={ "Expires On" }
...
export { Retouching_Agreements_Listing }
