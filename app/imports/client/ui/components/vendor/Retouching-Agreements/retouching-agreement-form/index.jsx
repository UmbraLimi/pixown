...
@inject('store')
@observer
class Retouching_Agreement_Form extends React.Component {
	constructor(props) {
		super(props)
	...
	render() {
		const { Manager:raManager, events, formStore } = this.props
		const pass_thru = {
			manager: raManager, 
			formStore: formStore,
		...
		return (
			<div>
				...
				<Field 	{...pass_thru} 					
						...
						label="Studio"
						tabIndex={this.tabIndexGen.next().value}
						title={Vendor_Masthead_Spec(raManager)}
				/>
				<Field 	{...pass_thru} 					
						id='title'
						label="Title"
...
export { Retouching_Agreement_Form }
