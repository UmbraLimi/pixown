...
class Header_Bar extends React.Component {
	...
	render() {
		...
		const logo_route = __.includes(['home', 'login'], current_view) ? 'home' : 'admin_homebase'
...
const Login_Button__ = (props) => {
  ...
  return (
    <Menu_Item_>
			<Login_Link_ 					view={app_routes.admin_homebase} store={props.store}>
        Login
      </Login_Link_>
    </Menu_Item_>
  )
...
class User_Dropdown__ extends React.Component {
  ...
  goHomebase = () => {
		...
		const { router } = store
		router.goTo(app_routes.admin_homebase, {}, store)
  }

  goProfile = () => {
		const { store } = this.props
		const { router } = store
		router.goTo(app_routes.admin_profile, {}, store)
  }
