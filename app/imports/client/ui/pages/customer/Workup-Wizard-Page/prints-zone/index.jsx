...
import { Prints_Zone_, Main_, Section_Row_,
...
// ----collections------------------
// ----components-------------------
import { Printing_Section }   	from  './printing-section/index.jsx'

// ====================================================================
@inject('store')
@observer
class Prints_Zone extends React.Component {
	...
	render() {
		...
		const pricing_ok = workupWizardStore._totalPriceForPrints_is_ok
		const price = workupWizardStore._totalPrice_for_prints
		const show_price = `${numeral(price/100).format('$ 0.00')}${pricing_ok ? '' : '?'}`

		return (
			<Prints_Zone_ 				style={{ zIndex: workupWizardStore.eyeMode_is_active ? -1 : 1 }} >
				<Title_Block_>
					<Title_>
						{
				...
				<Main_>
					<Section_Row_>
						<Section_Header_ >
							<Section_Title_	> { 'Prints' } </Section_Title_>
							<Total_Section_Cost_> 
								{ `Total: ${show_price}` }
							</Total_Section_Cost_>
						</Section_Header_>
