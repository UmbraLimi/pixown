...
// ====================================================================
if (Meteor.isServer) {
	const name = 'PRINTING_AGREEMENTS'
	// ------------------
	...
	export function printing_options_cursor__given__agreement_record (agreement) {
		const list = __.map(agreement.options_list, (item) => {
			return item.option_code
		})
		return set_cursor_on_server({	name: 'PRINTING_OPTIONS', 
			find_args: {
